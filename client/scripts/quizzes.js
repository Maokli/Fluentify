...
async function getCategoryQuizzes(){
  ...
  async function renderLanguageSelection() {
    ...
    */
    ...
    USER_LANGUAGES_NAMES.forEach(language => {
      const languageCard = createLanguageCard(USER_LANGUAGES_PHOTOS[USER_LANGUAGES_NAMES.indexOf(language)], language);
  ...
  async function getQuizzQuestionOptions(languageId, categoryId) {
    const response = await axiosInstance.get(`http://127.0.0.1:8000/api/quizz/byCategoryAndLanguage/${categoryId}/${languageId}`);
    const quizzes = response.data;
    quizzes.forEach(quizz => {
        QUIZZ_QUESTION.push(quizz.question);
        QUIZZ_ID.push(quizz.id);
        QUIZZ_OPTION1.push(quizz.option.substring(0,quizz.option.indexOf('/')));
        QUIZZ_OPTION2.push(quizz.option.substring(quizz.option.indexOf('/')+1,quizz.option.lastIndexOf('/')));
        QUIZZ_OPTION3.push(quizz.option.substring(quizz.option.lastIndexOf('/')+1));
        });
  }
  async function checkAnswer(quizzId,answer) {
    ...
    const response = await axiosInstance.post("http://127.0.0.1:8000/api/quizz/check", data);
...
async function renderQuizz() {
    await getQuizzQuestionOptions(USER_LANGUAGES_IDS[USER_LANGUAGES_NAMES.indexOf(window.language)], QUIZZ_CATEGORY_ID[QUIZZ_CATEGORY.indexOf(window.category)]);
    ...
    document.addEventListener('click', async (event) => {
      if (event.target.matches('.btn-primary')) {
        ...
        if (selectedOption) {
          const answer = selectedOption.parentElement.querySelector('.form-check-label').innerText;
